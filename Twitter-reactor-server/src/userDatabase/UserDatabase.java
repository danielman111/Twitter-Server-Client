...
public class UserDatabase {
	...
	public synchronized boolean logClientIn(String clientName, String passcode, ServerProtocol protocol){//ConnectionHandler<StompFrame> cH
		User userFound= _users.get(clientName);
		if ((userFound != null) && (userFound.checkPassword(passcode)) && (!userFound.isLoggedIn())){
			_users.get(clientName).setConnection(protocol);//cH
			return true;
		}
		return false;
	}
