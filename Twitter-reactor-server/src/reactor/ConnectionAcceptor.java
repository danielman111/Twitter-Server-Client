...
public class ConnectionAcceptor<T> {
	 ...
	 */
	public ConnectionAcceptor(ServerSocketChannel ssChannel, ReactorData<T> data, Reactor<T> server) {
	...
	public void accept() throws IOException {
		...
		if (sChannel != null) {
			...
			ConnectionHandler<T> handler = ConnectionHandler.create(sChannel, _data, key, _server);
			handler.switchToReadOnlyMode(); // set the handler to read only mode
			_data.addConnection(handler);
		}
	}
}
