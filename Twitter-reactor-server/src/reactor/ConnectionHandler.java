...
public class ConnectionHandler<T> {
	 ...
	 */
	private ConnectionHandler(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, Reactor<T> server) {
		_sChannel = sChannel;
		_data = data;
	...
	public static <T> ConnectionHandler<T> create(SocketChannel sChannel, ReactorData<T> data, SelectionKey key, Reactor<T> server) {
		ConnectionHandler<T> h = new ConnectionHandler<T>(sChannel, data, key, server);
		h.initialize();
		return h;
	}
	...
	public void addOutData(T message){
		ByteBuffer convertedMessage= null;
		try {
			convertedMessage = _tokenizer.getBytesForMessage(message);
		} catch (CharacterCodingException e) {
			e.printStackTrace();
		}
		addOutData(convertedMessage);
	}
